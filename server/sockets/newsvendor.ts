...
function calculateRound(
  config: GameConfig, day: number, order: number, demand: number, prevCumulativeProfit: number
): RoundResult {
  ...
  return {
    day, order, demand, sold, wasted, shortage,
    revenue, dailyCost, salvageValue, profit,
...
function getRoomState(room: GameRoom) {
  ...
  return {
    gameCode: room.gameCode, phase: room.phase, config: room.config,
    currentRound: room.currentRound, showingResults: room.showingResults,
    players, timerRemaining: room.timerRemaining,
    advanceMode: room.advanceMode, timerDuration: room.timerDuration,
  };
}
...
function getPlayerGameState(room: GameRoom, player: PlayerData) {
  ...
  const currentResult = room.showingResults && player.rounds.length > 0
    ...
    : null;
  return {
    gameCode: room.gameCode, playerName: player.name, phase: room.phase,
    config: { ...room.config, mu: 0, sigma: 0 },
    currentRound: room.currentRound, showingResults: room.showingResults,
    rounds: player.rounds, cumulativeProfit: player.cumulativeProfit,
    demands: revealedDemands, timerRemaining: room.timerRemaining,
    currentResult,
  };
}
...
function getDebriefData(room: GameRoom) {
  ...
  for (const [name, p] of room.players) {
    ...
    const avgOrder = orders.length > 0 ? orders.reduce((s, v) => s + v, 0) / orders.length : 0;
    players[name] = {
      name: p.name, rounds: p.rounds, totalProfit: p.cumulativeProfit,
      avgOrder, avgDemand: room.config.mu,
  ...
  return { gameCode: room.gameCode, config: room.config, demands: room.demands, players };
}

// ── Registration ───────────────────────────────────────

export function registerNewsvendorSockets(nsp: Namespace) {
  ...
  function sendPlayerState(room: GameRoom, player: PlayerData) {
    if (player.socketId) {
      nsp.to(player.socketId).emit('player-game-state', getPlayerGameState(room, player));
    }
  }
  ...
  function revealDemand(room: GameRoom) {
    ...
    for (const [, player] of room.players) {
      const order = player.hasSubmitted && player.currentOrder !== null
        ? player.currentOrder
        : Math.round(room.config.mu);
      const result = calculateRound(room.config, room.currentRound, order, demand, player.cumulativeProfit);
  ...
  function advanceToNextRound(room: GameRoom) {
    if (room.currentRound >= room.config.rounds) {
      ...
      if (room.instructorSocketId) {
        nsp.to(room.instructorSocketId).emit('debrief-data', debriefData);
      }
      for (const [, p] of room.players) {
        if (p.socketId) nsp.to(p.socketId).emit('debrief-data', debriefData);
      }
      return;
    }
    room.currentRound++;
  ...
  function checkAutoAdvance(room: GameRoom) {
    ...
    const activePlayers = Array.from(room.players.values()).filter((p) => p.socketId !== null);
  ...
  nsp.on('connection', (socket) => {
    console.log(`[newsvendor] Connected: ${socket.id}`);

    socket.on('create-game', (payload: any, callback: any) => {
      ...
      const fullConfig: GameConfig = {
        price: config.price ?? 6, cost: config.cost ?? 2, salvage: config.salvage ?? 0.5,
        mu: config.mu ?? 100, sigma: config.sigma ?? 30, rounds: config.rounds ?? 30,
        seed, mode: config.mode ?? 'medium',
      };
      const demands = generateDemand(seed, fullConfig.rounds, fullConfig.mu, fullConfig.sigma);
      const room: GameRoom = {
        gameCode, phase: 'lobby', config: fullConfig, demands,
        currentRound: 1, showingResults: false,
        players: new Map(), instructorSocketId: socket.id,
        advanceMode: advanceMode || 'manual', timerDuration: timerDuration || 30,
    ...
    socket.on('instructor-rejoin', (payload: any, callback: any) => {
    ...
    socket.on('reveal-demand', (gameCode: string) => {
      const room = rooms.get(gameCode);
      if (!room || room.phase !== 'playing' || room.showingResults) return;
      revealDemand(room);
    });

    socket.on('next-round', (gameCode: string) => {
      const room = rooms.get(gameCode);
      if (!room || room.phase !== 'playing' || !room.showingResults) return;
      advanceToNextRound(room);
    });
    ...
    socket.on('join-game', (payload: any, callback: any) => {
      ...
      const player: PlayerData = {
        name: trimmedName, socketId: socket.id,
        rounds: [], cumulativeProfit: 0, currentOrder: null, hasSubmitted: false,
      };
    ...
    socket.on('submit-order', (payload: any, callback: any) => {
    ...
    socket.on('request-debrief', (payload: any) => {
      const { gameCode } = payload;
      const room = rooms.get(gameCode);
      if (!room || room.phase !== 'debrief') return;
      socket.emit('debrief-data', getDebriefData(room));
    });

    socket.on('disconnect', () => {
      console.log(`[newsvendor] Disconnected: ${socket.id}`);
